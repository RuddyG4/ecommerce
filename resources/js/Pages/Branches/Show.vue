...
const disableSubmit = computed(() => {
    return (
        props.branch.branch_name !== form.branch_name ||
        props.branch.address !== form.address ||
        props.branch.city_id !== form.city_id
    );
});

function submit() {
    form.put(route('branches.update', { branch: props.branch.id }));
}

</script>

<template>
    <AppLayout title="Show branch">
        <template #header>
